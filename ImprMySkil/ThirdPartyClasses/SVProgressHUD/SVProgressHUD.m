...
@property (nonatomic, retain, readonly) UIView *gifView;
...
@synthesize overlayWindow, hudView, maskType, fadeOutTimer, stringLabel, imageView, gifView, visibleKeyboardHeight;
...
+ (void)showWithImageStatus:(NSString *)String{
    [[SVProgressHUD sharedView] showWithStatus:nil maskType:SVProgressHUDMaskTypeNone networkIndicator:NO];
}
...
+ (void)dismiss:(BOOL)isSVProgressHUD{
    if (isSVProgressHUD) {
        [[SVProgressHUD sharedView] dismiss];
    }
}
...
- (UIView *)gifView {
    if (gifView == nil) {
        gifView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, 81, 81)];
        gifView.center = CGPointMake(self.hudView.frame.size.width/2, self.hudView.frame.size.height/2);
        UIImageView *gifImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 81, 81)];

        NSMutableArray *gifArray = [[NSMutableArray alloc] init];
        for (int i = 0 ; i<= 84; i++) {
            NSString *string = [NSString stringWithFormat:@"loading_%d",i];
            [gifArray addObject:[UIImage imageNamed:string]];
        }
        gifImageView.animationImages = gifArray; //动画图片数组
        gifImageView.animationDuration = 3.5f; //执行一次完整动画所需的时长
